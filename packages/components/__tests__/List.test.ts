...
describe("List 虚拟列表", () => {
  ...
  it("只渲染可视区域内的条目，滚动后窗口跟随 scrollTop 移动", async () => {
    ...
    let rendered = wrapper.findAll(".u-list__item");
  ...
  it("滚到顶部时 startIndex 被钳制为 0", async () => {
    ...
    expect(wrapper.findAll(".u-list__item")[0].text()).toBe("0:item-0");
  });
  ...
  it("滚到底部时 endIndex 不超过 items.length", async () => {
    ...
    const rendered = wrapper.findAll(".u-list__item");
    expect(rendered).toHaveLength(4);
  ...
  it("containerHeight 支持 number / px / vh / 非法值", () => {
    // number
    expect(
      mountList({ containerHeight: 200 }, 100).findAll(".u-list__item"),
    ).toHaveLength(4);

    // px
    expect(
      mountList({ containerHeight: "200px" }, 100).findAll(".u-list__item"),
    ).toHaveLength(4);

    // vh：jsdom window.innerHeight 默认 768，10vh = 76.8 → ceil(76.8/50) = 2
    expect(
      mountList({ containerHeight: "10vh" }, 100).findAll(".u-list__item"),
    ).toHaveLength(2);

    // 非法值兜底 400 → ceil(400/50) = 8
    expect(
      mountList({ containerHeight: "abc" }, 100).findAll(".u-list__item"),
    ).toHaveLength(8);
  });
  ...
  it("itemKey 为字符串时从对象取字段", () => {
    ...
    expect(wrapper.findAll(".u-list__item")).toHaveLength(4);
  });
  ...
  it("同一帧内多次滚动只提交一次渲染", async () => {
    ...
    expect(wrapper.findAll(".u-list__item")[0].text()).toBe("3:item-3");
  });
  ...
  it("空数组时 scrollToIndex 不报错", async () => {
    ...
    expect(wrapper.findAll(".u-list__item")).toHaveLength(0);
  });
  ...
  it("数据减少时重新计算渲染窗口", async () => {
    ...
    expect(wrapper.findAll(".u-list__item")).toHaveLength(2);
    expect(wrapper.find(".u-list__phantom").attributes("style")).toContain(
      "height: 100px",
    );
  });
  ...
  it("buffer 会在可视区上下各多渲染", () => {
    ...
    expect(wrapper.findAll(".u-list__item")).toHaveLength(8);
  });
});
